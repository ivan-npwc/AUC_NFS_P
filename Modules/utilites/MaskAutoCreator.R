...
labelInput  <- "C:\\Users\\usato\\NFS_DB\\2024_138_OPP\\20240711_maskcreate"
UseAllHauloutImages=T
...
#  if (UseAllHauloutImages==F){
  ...
  MskDir=paste0(labelInput,"\\Predict\\Mask_Pred")   
# 
#  } 
...
# } 
  ...
  for (f in 1:length(listPreds)) {
    ...
    foreach(i = 1:length(listImageBl)) %dopar% {
      ...
      img0 <- t(mask0)
	  img0 <- t(img0)
     # dim(img0) <- c(dimModel, dimModel, 1)
      img = getFrame(img0, 1)
	...
	# }  
 stopCluster(cl)
  print(paste0("Done  ", f, "  pred from  " ,length(listImageBl))) 
     }
