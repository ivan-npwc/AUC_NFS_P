  ...
  HauloutDir1=paste0(labelInput,"\\Predict\\PRESENCE")
  
  
  MaskImgDir=paste0(labelInput,"\\", "Mask_Image"); dir.create(MaskImgDir,showWarnings = F)
  MaskImgDir0=paste0(labelInput,"\\", "Mask_Image0"); dir.create(MaskImgDir0,showWarnings = F)
  maskDir=paste0(labelInput,"\\", "Mask_Image", "\\","Mask"); dir.create(maskDir)
  imgDir=paste0(labelInput,"\\Mask_Image\\Image"); dir.create(imgDir,showWarnings = F)
  pthImg=paste0(labelInput,"\\",date1, "_CountDist.csv")
  
    if (file.exists(pthImg)==T){
...
imgsListFrom1=paste0(HauloutDir,"\\",imgsList)
imgsListFrom2=paste0(HauloutDir1,"\\",imgsList)

imgsListTo=paste0(imgDir,"\\",date1,"_",imgsList)
imgsListTo1=paste0(imgDir,"\\",date1,"_",imgsList)

file.copy(imgsListFrom1,imgsListTo,overwrite = F)
file.copy(imgsListFrom2,imgsListTo1,overwrite = F)
