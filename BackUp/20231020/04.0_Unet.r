					 ...
					 PTHweight = paste0(System_data,"\\weights\\NFSpup\\", NFS_Pup_weight_pth) # NFS_Pup_weight_pth
...
if(exists("unet1")==F) {source("Modules/04.1_Unet512Create.r")} 
   ...
   cl <- makePSOCKcluster(detectCores(logical=FALSE)-1) 
    clusterEvalQ(cl, {
     ...
     library(keras)
      ...
      blank=image_blank(512,512,color = "black")
      ...
      img2arr <- function(image, 
	                      dimModel1) {			  
		dimModel2<<-dimModel1
		if (as.character(image_info(image)[4])== "Gray") {
		      result <- aperm(as.numeric(blank[[1]])[, , 1:3], c(2, 1, 3))} else {
