...
#   BlobTable=BlobTable[BlobTable$m.cx != 0,]
  ...
  dimModel=vision_dimensions
  indexImgSize=   1024/dimModel 
  IndexBorderLeft=0.25*dimModel #0.25
  IndexBorderRight=0.75*dimModel #0.75
...
options(warn=0)	
    ...
    result_geo1 = data.frame(lat=result_geo$lat, lon=result_geo$lon,s.radius.max=result_geo$s.radius.max,s.area=result_geo$s.area)
	...
	Points <- SpatialPointsDataFrame (data.frame(rg$lon,rg$lat), data.frame(rg[,3:4]))
    proj4string(Points) <- crs
	Points =spTransform(Points,CRS(crs))
	
	
	dmat <- gWithinDistance(Points, byid=TRUE, dist=d)
